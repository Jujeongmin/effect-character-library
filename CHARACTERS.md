...
306장의 그림이 **172개 캐릭터 항목**으로 들어있다 (인간형 69 · 야수형 54 ·
동물형 7 · 몬스터형 42). 141개는 그림 한 장짜리, 31개는 여러 장을 탭으로
골라보는 B 형식(`animations`) — 뷰어에서 카드를 클릭하면 자동으로 탭이 뜬다.
...
- **베르단디·스쿨드·울드·고양이수인·페어리·루이·루시 (7명)** — 세로로 긴
  2단 레이아웃이라 자동 슬라이서가 열을 못 나눴다. 시트 통째로 한 장
  (`<id>_expressions`)만 들어있다 — 배경·글자 그대로.
