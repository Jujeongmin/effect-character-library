...
characters/
  ...
  items/                  아이템 — 캐릭터 몸에서 분리한 무기/장비 낱개 그림
```
...
캐릭터 PSD 안에 몸이랑 분리된 채로 들어있던 무기/지팡이/활 레이어를 따로
꺼낸 것 — 41개. 원본에 등장하는 캐릭터별로 하나씩이고, `id`는
`item_<원본 폴더 번호>` (예: `item_40100030` = 궁수의 활), `note` 필드에
어느 캐릭터 PSD의 어느 레이어에서 뽑았는지 적어뒀다. 캐릭터 항목과
자동으로 연결돼 있진 않다 — id 번호로 짐작만 가능.

일부(활/스태프 말고 둥근 방패·오브 형태)는 이름이 전부 "무기"로 붙어있는데
원본 레이어 이름이 그냥 `weapon`이라 그런 것 — 실제로는 방패에 가까운 것도
섞여 있다.

### 무기·아이콘 세트 (`weapon` / `skill_icon` / `equipment_icon`)
