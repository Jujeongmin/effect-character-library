...
306장의 그림이 **179개 캐릭터 항목**으로 들어있다 (인간형 76 · 야수형 54 ·
동물형 7 · 몬스터형 42). 155개는 그림 한 장짜리, 24개는 여러 장을 탭으로
골라보는 B 형식(`animations`) — 뷰어에서 카드를 클릭하면 자동으로 탭이 뜨고,
그리드 카드에도 미리 "N개" 표시가 붙어서 클릭 전에 여러 모습이 있는지 알 수 있다.
...
- **베르단디·스쿨드·울드·고양이수인·페어리·루이·루시 (7명)** — 세로로 긴
  2단 레이아웃이라 자동 슬라이서가 열을 못 나눴다. 시트 통째로 한 장인데,
  캐릭터 탭에 억지로 끼워 넣었더니 무대에 이상하게 나와서(제보받음) —
  탭이 아니라 **`<id>_expressions` 라는 별도 캐릭터 항목**으로 분리해 뒀다.
  본체(`lucy` 등)는 깨끗한 정지 그림 하나만 남고, 시트는 검색하면 따로 나온다.
